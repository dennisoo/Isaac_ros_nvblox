# Info
Da ich meinen PC neu aufgesetzt habe, und das mit dem prebuilt docker nicht geklappt hat, habe ich mal versucht jetzt mit dem offiziellen NVIDIA Ros development docker, alles sauber und wie von Nvidia vorgesehen aufzusetzen. Das Problem, dass wir derzeit haben, ist das wir basically viel selbst gebuildet haben, dies ist nicht von Nvidia vorgesehen und resultiert in einigen Problemen (Wie ihr wahrscheinlich gesehen habt xD). Ich schau mal wie weit ich komme, ich schreibe diesen Teil aktuell am 26.12.25.
Disclaimer: Ich habe mich versucht beim formattieren der Befehle zu bemühen, aber es kann natürlich sein, dass noch einige Befehle durch die Formatierung zerschossen wurden, schreibt einfach wenn ein Befehl sich unerwartet verhält.
...
## VSLAM
Ich versuche derzeit noch einige weiteren NVIDIA packages aus, diese könnten evtl. unseren Development-Prozess signifikant vereinfachen. Ich starte mal mit VSLAM, ein Package, welches die Odometrie mithilfe der Kamera berechnet, dies könnte uns später einiges an Arbeit ersparen.
Sicherstellen, das alle libaries da sind.

```
sudo apt-get install -y curl jq tar
```

Den nächsten Befehl kriege ich aktuell hier noch nicht drauf (kann man sich später drum kümmern) nehmt einfach den Befhel nach dem Update befehl auf dem Link:
https://nvidia-isaac-ros.github.io/repositories_and_packages/isaac_ros_visual_slam/isaac_ros_visual_slam/index.html
Nun gehen wir nochmal in die Dockerfile und builden anschließend:

```
sudo nano /etc/isaac-ros-cli/docker/Dockerfile.user_pkgs
```
